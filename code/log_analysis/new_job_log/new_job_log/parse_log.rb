...
module TestNum
      ...
      if @num_tests_run==0
          ...
          file_array.each_with_index do |line,index|
              ...
              if !(line =~ /(\d*) tests completed, (\d*) failed(, (\d*) skipped)?/).nil?
                ...
                @num_tests_run += $1.to_i
                @num_tests_failed += $2.to_i
                @num_tests_skipped += $4.to_i unless $3.nil?
            elsif !(line =~ /Total tests run:(\d+), Failures: (\d+)(, Skips: (\d+))?/).nil?
                  init_tests
              ...
              elsif !(line =~ /Total time: (.*)/).nil? and test_flag==1
  ...
  def self.save_maven_errors(user,repo)
    p "into save"
      ...
      test_id=[]
      Cll_failedtest.where("id>=3000000").find_each do |test|
          test_id << test.id
      end
      
      Job.where("id>3000000 and test_failed = true").find_each do |job|
