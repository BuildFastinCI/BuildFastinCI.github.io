...
module TestNum
  ...
  def self.parse_test(flag,file_array,jobs_id)
    ...
    if flag=='maven' 
      ...
      if @num_tests_run==0
          ...
          test_flag=0
          file_array.each do |line|
          if !(line =~ /\A(test|Test|integrationTest)/).nil? 
            test_flag=1
          end
          if !(line =~ /(There were failing tests)|(test FAILED)/).nil? 
              init_tests
              test_flag=1
              @test_failed = true
          end
    ...
    # p job_info
    begin
        cll_failed=Cll_failedtest.new(job_info)
        cll_failed.save
    rescue => exception
        p exception
    end
  ...
  def self.init_save_maven_errors
    ...
    #         bulk << Cll_failedtest.new(hash)
    #       end
    #       Cll_failedtest.import bulk
    #       p "imported"
    #       # ActiveRecord::Base.clear_active_connections!
    #       break if hash == :END_OF_WORK
    #     end
      ...
      @thread_num.times do 
        thread = Thread.new do
          loop do
            hash = @inqueue.deq
            break if hash == :END_OF_WORK
            # if $test_id.include? hash[:id]
            #   $test_id=$test_id-[hash[:id]]
            #   p $test_id.size
            #   break
            # end
            extract_test hash
  ...
  def self.save_maven_errors(user,repo)
    ...
    threads=init_save_maven_errors
      ...
      test_id=[]
      Cll_failedtest.where("id>=3300000").find_each do |test|
          test_id << test.id
      end
      
      Job.where("id>3300000 and test_failed = true").find_each do |job|
        hash = Hash.new
        if test_id.size>0 and test_id.include? job.id
...
repo_name='AuthorizeNet/sdk-java'
job_number='814.1'
log_path = File.expand_path(File.join('..', '..', '..', 'bodyLog2', 'build_logs', repo_name.sub(/\//,'@'), job_number.sub(/\./, '@')+'.log'), File.dirname(__FILE__))
p log_path
