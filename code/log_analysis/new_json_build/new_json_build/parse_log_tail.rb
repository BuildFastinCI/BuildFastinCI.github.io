...
require_relative'logtype_extract'
...
module TestNum
  ...
  def self.parse_test(flag,file_array,jobs_id)
    ...
    if flag=='maven' 
      ...
      if @num_tests_run==0
          ...
          if !(line2 =~ /(\d*) tests completed, (\d*) failed(, (\d*) skipped)?/).nil?
              p "tests completed"
              init_tests 
              @num_tests_run += $1.to_i
              @num_tests_failed += $2.to_i
  ...
  def self.save_maven_errors(user,repo)
      ...
      # @inqueue.enq hash
      test_id=[]
      Cll_test.where("id>=3006809").find_each do |test|
          test_id << test.id
      end
      
      Job.where("id>=3006809").find_each do |job|
        hash = Hash.new
